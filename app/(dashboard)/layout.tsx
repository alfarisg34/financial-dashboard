...
import {
  ...
  ChevronLeft, ChevronRight, LogOut, Menu, X, Sparkles
} from 'lucide-react'
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    <>
      ...
      <div className="p-3 border-t border-slate-800/80">
        <button onClick={handleLogout}
          className="flex items-center gap-3.5 px-3.5 py-3 rounded-xl text-sm font-medium text-slate-400 hover:bg-rose-500/10 hover:text-rose-400 hover:border hover:border-rose-500/20 w-full transition-all duration-200">
  ...
  return (
    <div className="flex h-screen bg-[#090d16] text-slate-100 overflow-hidden">
      ...
      <aside className={`fixed top-0 left-0 h-full w-64 bg-[#0f172a]/95 backdrop-blur-xl border-r border-slate-800/80 flex flex-col z-50 transition-transform duration-300 lg:hidden
      ...
      {/* Desktop sidebar */}
      <aside className={`hidden lg:flex ${collapsed ? 'w-20' : 'w-60'} bg-[#0d1322]/80 backdrop-blur-xl border-r border-slate-800/70 flex-col transition-all duration-300 shrink-0`}>
        <div className="flex items-center justify-between p-4.5 border-b border-slate-800/70">
          ...
          <button onClick={() => setCollapsed(!collapsed)}
            className="p-2 rounded-xl hover:bg-slate-800/80 text-slate-400 hover:text-slate-200 ml-auto transition-colors">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Mobile topbar */}
        <div className="lg:hidden flex items-center justify-between px-4 py-3.5 bg-[#0d1322]/90 backdrop-blur-md border-b border-slate-800/80 shrink-0">
